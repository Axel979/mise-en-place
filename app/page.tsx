...
/* ═══ CHALLENGES TAB ══════════════════════════════════════════════════════ */
function ChallengeDetail({ch,progress,onBack,onInvite,friends}){
  ...
  // Friends doing the same challenge (real data if available, else mock)
  const friendsOnChallenge=(friends||[
    {username:"Sofia R.",prog:Math.min(ch.target,prog+2),xp:620},
    {username:"Jake M.",prog:Math.min(ch.target,Math.floor(prog*0.7)),xp:310},
  ]).slice(0,5);

  return(
    <div style={{background:C.paper,minHeight:"100vh"}}>
      {/* Hero */}
      <div style={{background:`linear-gradient(160deg,${ch.color},${ch.dark||ch.color+"BB"})`,padding:"20px 20px 32px",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",right:-20,top:-20,width:160,height:160,borderRadius:"50%",border:"32px solid rgba(255,255,255,.06)",pointerEvents:"none"}}/>
        ...
        <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
          ...
          <span style={{background:"rgba(255,255,255,.18)",borderRadius:6,padding:"3px 10px",fontSize:11,fontWeight:700,color:"#fff"}}>+{ch.xp} 🔥 Heat</span>
        ...
        <div style={{fontWeight:900,fontSize:26,color:"#fff",fontFamily:DF,marginBottom:6,lineHeight:1.2}}>{ch.name}</div>
        <div style={{fontSize:14,color:"rgba(255,255,255,.8)",lineHeight:1.6}}>{ch.tagline}</div>
      </div>
      ...
      <div style={{padding:"20px 16px 100px"}}>
        ...
        <div style={{background:C.cream,borderRadius:20,padding:18,marginBottom:12,border:`1px solid ${C.border}`}}>
          ...
          <div style={{background:C.border,borderRadius:99,height:8,overflow:"hidden",marginBottom:14}}>
            <div style={{width:`${pct}%`,height:"100%",background:ch.color,borderRadius:99,transition:"width .8s ease"}}/>
          </div>
          {/* Milestones */}
          <div style={{position:"relative",height:44,marginBottom:4}}>
            ...
            <div style={{position:"absolute",top:12,left:0,width:`${pct}%`,height:2,background:ch.color,transition:"width .8s ease"}}/>
            {(ch.milestones||[]).map((m,i)=>{
              ...
              return(
                <div key={i} style={{position:"absolute",left:`${mp}%`,transform:"translateX(-50%)",top:0}}>
                  <div style={{width:24,height:24,borderRadius:"50%",background:reached?ch.color:C.border,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:10,fontWeight:800,margin:"0 auto 4px",transition:"all .3s"}}>{reached?"✓":m}</div>
          ...
          {done&&(
            <div style={{background:`${C.gold}15`,borderRadius:12,padding:"12px",textAlign:"center",marginTop:8}}>
              <div style={{fontWeight:800,fontSize:14,color:C.bark}}>🏅 Challenge Complete! +{ch.xp} 🔥 Heat earned</div>
            </div>
          )}
          {!done&&nextM&&(
            <div style={{fontSize:12,color:C.muted,marginTop:4}}>{nextM-prog} more to next milestone</div>
          )}
        </div>
        ...
        <div style={{background:C.cream,borderRadius:20,padding:18,marginBottom:12,border:`1px solid ${C.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div style={{fontWeight:900,fontSize:15,color:C.bark,fontFamily:DF}}>Friends on this challenge</div>
          ...
          {friendsOnChallenge.length===0?(
            <div style={{textAlign:"center",padding:"16px 0"}}>
              <div style={{fontSize:13,color:C.muted,marginBottom:10}}>No friends on this challenge yet</div>
              <button onClick={()=>setShowInvite(true)} style={{padding:"8px 20px",borderRadius:10,border:`1.5px solid ${ch.color}`,background:"transparent",color:ch.color,fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>Challenge a friend</button>
            </div>
          ):(
            friendsOnChallenge.map((f,i)=>{
              const fp=Math.min(f.prog||0,ch.target);
              const fpct=Math.round(fp/ch.target*100);
              return(
                <div key={i} style={{display:"flex",alignItems:"center",gap:12,marginBottom:i<friendsOnChallenge.length-1?12:0}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{background:C.border,borderRadius:99,height:5,overflow:"hidden"}}>
                      <div style={{width:`${fpct}%`,height:"100%",background:ch.color,borderRadius:99,transition:"width .5s"}}/>
                    </div>
                  </div>
                </div>
              );
        ...
        {/* About */}
        <div style={{background:C.cream,borderRadius:20,padding:18,marginBottom:12,border:`1px solid ${C.border}`}}>
          <div style={{fontWeight:900,fontSize:15,color:C.bark,marginBottom:10,fontFamily:DF}}>About</div>
          <p style={{fontSize:14,color:"#6A5C52",lineHeight:1.75,margin:0}}>{ch.about}</p>
        </div>

        {/* Learn */}
        <div style={{background:`${C.sky}0D`,border:`1px solid ${C.sky}28`,borderRadius:20,padding:18,marginBottom:12}}>
        ...
        {/* Tips */}
        <div style={{background:`${C.gold}12`,border:`1px solid ${C.gold}35`,borderRadius:20,padding:18,marginBottom:20}}>
        ...
        <button onClick={onBack} style={{width:"100%",padding:"14px",borderRadius:14,border:"none",background:ch.color,color:"#fff",fontWeight:800,fontSize:15,cursor:"pointer",fontFamily:"inherit"}}>
        ...
        <Sheet onClose={()=>setShowInvite(false)}>
          <div style={{padding:"24px 20px 44px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
              <div>
                <div style={{fontWeight:900,fontSize:20,color:C.bark,fontFamily:DF}}>Challenge a Friend</div>
                <div style={{fontSize:12,color:C.muted,marginTop:2}}>They'll see your progress side by side</div>
              </div>
            ...
            {["Sofia R.","Jake M.","Priya K.","Marcus T."].map(name=>(
              <button key={name} onClick={()=>{onInvite&&onInvite(name,ch);setShowInvite(false);}} style={{display:"flex",alignItems:"center",gap:14,background:C.cream,border:`1.5px solid ${C.border}`,borderRadius:16,padding:"13px 16px",cursor:"pointer",textAlign:"left",width:"100%",marginBottom:10,fontFamily:"inherit"}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:11,color:C.muted}}>Tap to send challenge invite</div>
                </div>
...
function ChallengesTab({challengeProgress,onInvite,seasonalEvent}){
  ...
  if(selected){
    const ch=allChallenges.find(c=>c.id===selected);
    return <ChallengeDetail ch={ch} progress={challengeProgress[selected]||0} onBack={()=>setSelected(null)} onInvite={onInvite}/>;
  }
  ...
  const monthlyDone=monthlyProgress>=monthly.target;
  ...
  const Tile=({ch,progress=0})=>{
    ...
    const done=prog>=ch.target;
    return(
      <button onClick={()=>setSelected(ch.id)} className="tap" style={{background:C.cream,border:`1.5px solid ${done?ch.color:C.border}`,borderRadius:20,padding:"16px",cursor:"pointer",textAlign:"left",fontFamily:"inherit",position:"relative",overflow:"hidden",width:"100%",display:"block",transition:"box-shadow .15s",boxShadow:done?`0 0 0 2px ${ch.color}44`:"none"}}>
        {/* Colour accent top bar */}
        <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:done?ch.color:pct>0?ch.color+"88":"transparent",transition:"all .3s"}}/>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:11,color:done?ch.color:C.muted,fontWeight:done?700:400}}>
            {done?"✓ Complete":pct>0?`${prog}/${ch.target} ${ch.unit}`:`0/${ch.target} ${ch.unit}`}
          </span>
  ...
  return(
    <div style={{paddingBottom:40}}>
      <div style={{padding:"4px 16px 16px"}}>
        ...
        {/* Monthly challenge — always first */}
        <div style={{marginBottom:8}}>
          ...
          <Tile ch={monthly} progress={monthlyProgress}/>
        </div>

        {/* Active challenges */}
        {active.length>0&&(
          <div style={{marginBottom:8}}>
            <div style={{fontSize:10,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:".1em",margin:"18px 0 10px"}}>In Progress</div>
        ...
        {available.length>0&&(
          <div style={{marginBottom:8}}>
            <div style={{fontSize:10,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:".1em",margin:"18px 0 10px"}}>All Challenges</div>
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
        ...
        {completed.length>0&&(
          <div>
            <div style={{fontSize:10,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:".1em",margin:"18px 0 10px"}}>Completed</div>
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
...
function CommunityTab({allRecipes,onOpen,onSaveToLibrary}){
  ...
  const COMMUNITY_RECIPES=[
    {id:"c1",name:"Grandma's Carbonara",emoji:"",
     ...
     author:"Sofia R.",avatar:"👩‍🍳",rating:4.8,saves:124,category:"Italian",difficulty:"Medium",time:"25 min",xp:75,
    ...
    {id:"c2",name:"Spicy Miso Ramen",emoji:"",
     ...
     author:"Jake M.",avatar:"🧑‍🍳",rating:4.9,saves:89,category:"Japanese",difficulty:"Hard",time:"2 hrs",xp:125,
    ...
    {id:"c3",name:"Mum's Butter Chicken",emoji:"",
     ...
     author:"Priya K.",avatar:"👩‍🦱",rating:4.7,saves:203,category:"Indian",difficulty:"Medium",time:"1 hr",xp:85,
    ...
    {id:"c4",name:"Crispy Roast Potatoes",emoji:"",
     ...
     author:"Marcus T.",avatar:"",rating:4.6,saves:156,category:"Comfort",difficulty:"Easy",time:"1h 20m",xp:40,
    ...
    {id:"c5",name:"Açaí Power Bowl",emoji:"",
     ...
     author:"Yuki A.",avatar:"👩",rating:4.5,saves:67,category:"Healthy",difficulty:"Easy",time:"10 min",xp:38,
    ...
    {id:"c6",name:"Tres Leches Cake",emoji:"",
     ...
     author:"Liam B.",avatar:"👨",rating:4.8,saves:112,category:"Baking",difficulty:"Medium",time:"1 hr",xp:66,
    ...
    {id:"c7",name:"Thai Basil Fried Rice",emoji:"",
     ...
     author:"Sofia R.",avatar:"👩‍🍳",rating:4.6,saves:98,category:"Asian",difficulty:"Easy",time:"15 min",xp:28,
    ...
    {id:"c8",name:"Shakshuka with Feta",emoji:"",
     ...
     author:"Priya K.",avatar:"👩‍🦱",rating:4.7,saves:145,category:"Breakfast",difficulty:"Easy",time:"25 min",xp:28,
...
function StreakCalendar({cookedDays, cookLog, onClose}){
  ...
  const getFirstDay=(m,y)=>new Date(y,m,1).getDay(); // 0=Sun

  // Build a set of cooked date strings from cookLog + current week
  const cookedSet = useMemo(()=>{
    const dates = new Set();
    // From cook log (real data)
    (cookLog||[]).forEach(entry=>{
      if(entry.date){
        // entry.date is like "8 Apr 2026"
        try{
          const d=new Date(entry.date);
          if(!isNaN(d)) dates.add(`${d.getFullYear()}-${d.getMonth()}-${d.getDate()}`);
        }catch{}
      }
    });
    // From current week cookedDays array
    const now=new Date();
  ...
  const firstDay=getFirstDay(viewMonth,viewYear); // 0=Sun
  ...
  const currentStreak=useMemo(()=>{
    ...
    while(true){
      ...
      if(cookedSet.has(key)){streak++;d.setDate(d.getDate()-1);}
      else break;
    }
    return streak;
  },[cookedSet]);
  ...
  // Check if two adjacent days are both cooked (for streak connector)
  ...
  const isStreakEnd=(day)=>{
    ...
    return !cookedSet.has(nextKey);
  };

  const MILESTONES=[
    {days:7,  label:"Week Warrior",   desc:"Cook 7 days in a row",     icon:"🔥"},
    {days:14, label:"Fortnight Chef", desc:"Cook 14 days in a row",    icon:"⚡"},
    {days:30, label:"Month Master",   desc:"Cook every day for a month",icon:"👑"},
    {days:100,label:"Legend",         desc:"100 day streak",            icon:"💎"},
  ];
    ...
    <Sheet onClose={onClose}>
      <div style={{padding:"0 0 44px"}}>
        ...
        <div style={{padding:"24px 20px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{fontWeight:900,fontSize:20,color:C.bark,fontFamily:DF}}>Streak Calendar</div>
          <CloseBtn onClose={onClose}/>
        </div>

        {/* Streak hero */}
        <div style={{margin:"0 16px 20px",background:`linear-gradient(145deg,${C.bark},#3D2010)`,borderRadius:20,padding:"18px 20px",color:"#fff",display:"flex",alignItems:"center",gap:16}}>
          <div style={{fontSize:52,lineHeight:1}}>🔥</div>
          <div>
            <div style={{fontWeight:900,fontSize:32,lineHeight:1,fontFamily:DF}}>{currentStreak}</div>
            <div style={{fontSize:13,opacity:.7,marginTop:2}}>day streak</div>
          </div>
          <div style={{marginLeft:"auto",textAlign:"right"}}>
            <div style={{fontWeight:800,fontSize:16}}>{totalCooked}</div>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px",marginBottom:12}}>
          <button onClick={prevMonth} style={{background:"none",border:"none",cursor:"pointer",padding:"8px",borderRadius:10,color:C.bark,fontSize:18}}>‹</button>
          <div style={{fontWeight:800,fontSize:16,color:C.bark}}>{monthName}</div>
          <button onClick={nextMonth} disabled={!canGoNext} style={{background:"none",border:"none",cursor:canGoNext?"pointer":"default",padding:"8px",borderRadius:10,color:canGoNext?C.bark:C.border,fontSize:18}}>›</button>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",padding:"0 16px",gap:"4px 0"}}>
          ...
          {Array.from({length:daysInMonth}).map((_,i)=>{
            ...
            return(
              <div key={day} style={{position:"relative",display:"flex",alignItems:"center",justifyContent:"center",height:44}}>
                {/* Streak connector bar */}
                {isCooked&&!streakStart&&!isFirstCol&&(
                  <div style={{position:"absolute",left:0,right:"50%",top:"50%",transform:"translateY(-50%)",height:32,background:`${C.flame}25`,zIndex:0}}/>
                )}
                {isCooked&&!streakEnd&&!isLastCol&&(
                  <div style={{position:"absolute",left:"50%",right:0,top:"50%",transform:"translateY(-50%)",height:32,background:`${C.flame}25`,zIndex:0}}/>
                )}
                {/* Day circle */}
                <div style={{
                  ...
                  {isCooked&&<div style={{fontSize:8,lineHeight:1}}>🔥</div>}
                </div>
              </div>
            );
          })}
        ...
        <div style={{padding:"24px 16px 0"}}>
          <div style={{fontWeight:900,fontSize:18,color:C.bark,fontFamily:DF,marginBottom:4}}>Streak Society</div>
          <div style={{fontSize:12,color:C.muted,marginBottom:16}}>Keep cooking to unlock exclusive rewards</div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {MILESTONES.map(m=>{
              const unlocked=currentStreak>=m.days;
              const progress=Math.min(100,Math.round(currentStreak/m.days*100));
              return(
                <div key={m.days} style={{background:unlocked?`${C.flame}08`:C.cream,borderRadius:16,padding:"14px 16px",border:`1.5px solid ${unlocked?C.flame:C.border}`,display:"flex",alignItems:"center",gap:14}}>
                  <div style={{width:44,height:44,borderRadius:13,background:unlocked?`${C.flame}20`:"#E8E0D8",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0,filter:unlocked?"none":"grayscale(1)",opacity:unlocked?1:.5}}>
                    {m.icon}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontWeight:800,fontSize:14,color:unlocked?C.bark:C.muted}}>{m.label}</div>
                    ...
                    {!unlocked&&(
                      <div style={{marginTop:6}}>
                        <div style={{background:C.border,borderRadius:99,height:4,overflow:"hidden"}}>
                          <div style={{width:`${progress}%`,height:"100%",background:C.flame,borderRadius:99,transition:"width .5s"}}/>
                        </div>
                        <div style={{fontSize:10,color:C.muted,marginTop:3}}>{currentStreak}/{m.days} days</div>
                      </div>
                    )}
                  </div>
                  {unlocked&&<div style={{fontSize:11,fontWeight:700,color:C.flame,flexShrink:0}}>Unlocked!</div>}
                  {!unlocked&&<div style={{width:24,height:24,borderRadius:"50%",background:C.pill,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={C.muted} strokeWidth="2.5"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
                  </div>}
                </div>
              );
            })}
