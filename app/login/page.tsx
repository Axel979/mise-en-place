    ...
    <div style={{minHeight:'100vh',background:'#FAF4EE',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Georgia,serif',padding:'24px'}}>
      <div style={{width:'100%',maxWidth:400}}>
        <div style={{textAlign:'center',marginBottom:40}}>
          ...
          <div style={{fontSize:28,fontWeight:900,color:'#3B2A1A'}}>
        ...
        {sent ? (
          <div style={{textAlign:'center',background:'#FFF8F0',borderRadius:20,padding:'32px 24px',border:'2px solid #EEE5DC'}}>
            ...
            <div style={{fontSize:14,color:'#9E8C7E',lineHeight:1.6}}>
              We sent a magic link to <strong>{email}</strong>. Click it to sign in.
            </div>
            <button onClick={()=>{setSent(false);setEmail('');}} style={{marginTop:16,background:'none',border:'none',color:'#FF4D1C',fontWeight:700,cursor:'pointer',fontSize:14}}>
          ...
          <div style={{background:'#FFF8F0',borderRadius:20,padding:'32px 24px',border:'2px solid #EEE5DC'}}>
            <form onSubmit={signInWithEmail}>
              <div style={{marginBottom:12}}>
                <label style={{fontSize:12,fontWeight:700,color:'#9E8C7E',display:'block',marginBottom:6}}>
                  EMAIL ADDRESS
                </label>
                <input
              ...
              {error&&<div style={{fontSize:13,color:'#FF4D1C',marginBottom:12,padding:'8px 12px',background:'#FF4D1C18',borderRadius:10}}>{error}</div>}
              <button type="submit" disabled={loading||!email}
                style={{width:'100%',padding:'13px',borderRadius:14,border:'none',background:loading||!email?'#D8D0C8':'#FF4D1C',color:'#fff',fontWeight:800,fontSize:15,cursor:loading||!email?'not-allowed':'pointer',transition:'all .18s'}}>
